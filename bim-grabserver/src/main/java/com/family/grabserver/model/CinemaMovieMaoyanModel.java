...
public class CinemaMovieMaoyanModel implements AfterExtractor {
    ...
    private String source = "http://m.maoyan.com/showtime/wrap.json?cinemaid=**";

    public static void main(String[] args) {
        OOSpider.create(Site.me().setSleepTime(1000).addCookie("ci", "22")
                , new ConsolePageModelPipeline(), CinemaMovieMaoyanModel.class)
                .addPipeline(new JsonFilePipeline("D:\\grab\\"))
                .addPipeline(new ConsolePipeline())
                .addUrl("http://m.maoyan.com/showtime/wrap.json?cinemaid=48").thread(5).run();
